...
var shouldRestart = (a, b) => {
    for (var k in {
        ...
        if (typeof (isArray((b[k] = a[k])) ? b[k][0] : b[k]) === "function") {} else if (a[k] !== b[k]) return true
    }
}

var patchSubs = (oldSubs, newSubs, dispatch) => {
    for (
        var subs = [], i = 0, oldSub, newSub; i < oldSubs.length || i < newSubs.length; i++
    ) {
        oldSub = oldSubs[i]
        newSub = newSubs[i]
...
var createNode = (vdom, listener, isSvg) => {
    ...
    var node =
        ...
        document.createElementNS(SVG_NS, vdom.type, {
            is: props.is
        }) :
